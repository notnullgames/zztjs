export function ZZTTile (typeid, color) {
  this.typeid = typeid
  this.color = color
...
export function makeTile (typeid, color) {
  if (typeid == 0) { return _ZZTBoard_BoardEmpty } else { return new ZZTTile(typeid, color) }
}

export function ZZTBoard () {
  this.actorIndex = 0
  this.tick = 0
}
...
ZZTBoard.prototype.get = function (x, y) {
  if (!this.withinBoard(x, y)) { return _ZZTBoard_BoardEdge } else { return this.tiles[y * this.width + x] }
}
